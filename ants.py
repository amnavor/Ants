...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        if self.exit:
            self.exit.entrance=self #self is the existence of place (in this example)
            #self.exit is your place0, so you can isolate the entrance of place0 (which is place1)
                #aka uou take the entrance of that exit and set it to equal place
            #If the Place has an exit, then the exits entrance is set to that Place
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant.container: #if your current ant is a bodyguard
                if insect.container: #and if the insect you wanna remove is that bodyguard
                    self.ant=insect.ant #the new current ant is the ant bodyguard used to have
                if not insect.container: #if the insect you want to remove is not the container
                ...
                if type(self.ant)==QueenAnt:
                    ...
                    self.ant=None
            # make sure that the insect is this self.ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        current_range=0
        while location!= None:
            if self.min_range<=current_range and current_range<=self.max_range:
                if location.bees and location!=hive:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        place, damage=self.place, self.damage
...
class BodyguardAnt(Ant):
    ...
    food_cost=4
    implemented = True 
    container =True # Change to True when finished, to view in GUI
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony): #only have to redo action since everything is inherited from bodyguard, but we;re overriding action function
        for bee in self.place.bees[:]:
            bee.reduce_armor(self.damage)
        if self.ant:
            self.ant.action(colony)
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self):
        ...
        self.count=QueenAnt.total_queens
    ...
    def action(self, colony):
        ...
        if self.count>1:
            ...
            if isinstance(colony.queen, QueenPlace):
                QueenPlace.ant_queen=self.place
            else:
                colony.queen=QueenPlace(colony.queen, self.place)
            self.throw_at(self.nearest_bee(colony.hive))
            location, ants_with=self.place, []
            while location:
                if location.ant:
                    ants_with+=[location.ant]
                location=location.exit
            for ant in ants_with[:]:
                if ant.container:
                    if ant.ant:
                        if ant.ant not in QueenAnt.buffed_ants and ant.ant.name!='Queen':
                            ant.ant.damage=ant.ant.damage*2
                            QueenAnt.buffed_ants+=[ant.ant] 
                if ant not in QueenAnt.buffed_ants and ant.name!='Queen':
                    ant.damage=ant.damage*2
                    QueenAnt.buffed_ants+=[ant] 
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    bee.current_duration=0
    bee.action
    new_action=effect(bee.action)
    while bee.current_duration<duration:
        bee.action=bee.new_action(self, colony)
        bee.current_duration+=1
